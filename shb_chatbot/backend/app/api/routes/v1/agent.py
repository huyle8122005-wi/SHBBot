...
async def agent_websocket(
    websocket: WebSocket,
    user: User | None = Depends(get_current_user_ws_optional),
) -> None:
    ...
    """
    # Auth is handled by get_current_user_ws_optional dependency
    # If user is None, we proceed in guest mode (no DB persistence)
    ...
    try:
        while True:
            ...
            try:
                ...
                if user and current_conversation_id and agent_run.result:
                    try:
                        async with get_db_context() as db:
                            conv_service = get_conversation_service(db)
